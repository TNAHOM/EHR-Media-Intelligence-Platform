...
class SearchService:
    ...
    def _date_to_int(dt: date | str) -> int:
        ...
        if isinstance(dt, str):
            dt_str = dt[:10].replace("-", "")
            return int(dt_str) if dt_str.isdigit() else 20240101
        return int(dt.strftime("%Y%m%d"))

    @classmethod
    def index_all_records(cls, session: Session) -> IndexStats:
        """
        Extracts DocumentReferences, DiagnosticReports from FHIR Bundles,
        and ClinicalSummaries from SQLite, generates embeddings, and indexes into ChromaDB.
        """
        # 1. Reset vector collection to guarantee idempotency
        ...
        # A. Index FHIR Bundles (Notes, Labs, Imaging)
        bundles = session.exec(select(FHIRBundle)).all()
        for b in bundles:
            bundle_dict = json.loads(b.bundle_json)
            ...
            for entry in entries:
                ...
                if res_type == "DocumentReference":
                    ...
                    category = (
                        "imaging" if "RAD" in str(res.get("category", [])) else "lab"
                    )
        ...
        # B. Index AI Clinical Summaries (Task 3)
        ...
        for s in summaries:
            ...
            doc_ids.append(f"summary_{s.patient_mrn}_{s.id}")
            doc_texts.append(passage)
            doc_metadatas.append(
                {
                    "source_id": f"summary-{s.patient_mrn}",
